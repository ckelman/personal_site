...
private int state;
    public SuicideEnemy(int x, int y, int w, int h, int health)
    {
        
        super(x,y,w,h,health,null);
        boolean isright = true;
        state = 0;
    }
    public void attack(MainCharacter main, ArrayList<Platform> g)
    {
        Platform senseRadii = new Platform((int)(getX()-15),(int)(getY()-40),getWidth()+30,getHeight()+40);
        if(main.intersects(senseRadii))
        {
           Platform blastradius = new Platform((int)(getX()-60),(int)(getY()-80),200,200);
            main.setHealth(main.getHealth()-50);
            setHealth(0);
             if(!main.isPlatformRight(g))
             {
                   
                    main.setY(main.getY()-50);
                }
        }
    }
...
//State 0 StandRight, 1 StandLeft
        ...
        g2.setColor(Color.black);
        g2.drawRect(getXInt(),getYInt()-8,(int)(getHealth()/100*getWidth()),5);
